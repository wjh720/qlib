...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Deprecated compatibility arg; the QA-compatible implementation is vendored in qlib.",
    )
    parser.add_argument(
        "--factor-cache-dir",
        default="",
        help="Optional factor MD5 cache directory used by the vendored QA-compatible feature pipeline.",
    )
    parser.add_argument(
        "--disable-factor-cache",
        action="store_true",
        help="Force recomputing vendored QA-compatible features instead of reading factor caches.",
    )
    parser.add_argument(
        "--persist-factor-cache",
        action="store_true",
        help="Persist newly recomputed vendored QA-compatible features into the factor cache.",
    )
    parser.add_argument(
        "--raw-feature-cache",
        default="",
        help="Optional pickle path for the legacy/raw feature matrix.",
    )
    parser.add_argument(
        "--qa-feature-cache",
        default="",
        help="Optional pickle path for the vendored QA-compatible feature matrix.",
    )
    parser.add_argument(
...
def resolve_cache_path(raw_path: str) -> Path | None:
    if not raw_path:
        return None
    return Path(raw_path).expanduser().resolve()


def maybe_extract_feature_group(feature_df: pd.DataFrame) -> pd.DataFrame:
    if isinstance(feature_df.columns, pd.MultiIndex):
        level0 = feature_df.columns.get_level_values(0)
        if "feature" in level0:
            return feature_df["feature"].copy()
    return feature_df


def maybe_load_cached_feature(cache_path: Path | None, tag: str) -> pd.DataFrame | None:
    if cache_path is None or not cache_path.exists():
        return None
    feature_df = pd.read_pickle(cache_path)
    feature_df = maybe_extract_feature_group(feature_df)
    feature_df = qa_ablation.normalize_index(feature_df)
    print(f"loaded {tag} feature cache from {cache_path}", flush=True)
    return qa_ablation.to_float32(feature_df)


def maybe_save_feature_cache(feature_df: pd.DataFrame, cache_path: Path | None, tag: str) -> None:
    if cache_path is None:
        return
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    qa_ablation.normalize_index(feature_df).to_pickle(cache_path)
    print(f"saved {tag} feature cache to {cache_path}", flush=True)
...
def build_feature_matrices(
    ...
    raw_cache_path = resolve_cache_path(args.raw_feature_cache)
    qa_cache_path = resolve_cache_path(args.qa_feature_cache)

    raw_feature_df = maybe_load_cached_feature(raw_cache_path, "raw")
    if raw_feature_df is None:
        raw_feature_df, _ = workflow._build_legacy_feature_label_frames(factors, field_panels)
        raw_feature_df = preprocess_raw_feature(raw_feature_df)
        maybe_save_feature_cache(raw_feature_df, raw_cache_path, "raw")

    qa_feature_df = maybe_load_cached_feature(qa_cache_path, "qa")
    qa_metadata: dict = {}
    if qa_feature_df is None:
        qa_feature_df, _qa_label_unused, qa_metadata = workflow._build_quantaalpha_feature_label_frames(factors, args)
        qa_feature_df = preprocess_qa_feature(qa_feature_df)
        maybe_save_feature_cache(qa_feature_df, qa_cache_path, "qa")

    return raw_feature_df, qa_feature_df, qa_metadata
...
def build_summary(
    ...
    return {
        ...
        "settings": {
            ...
            "raw_feature_cache": str(resolve_cache_path(args.raw_feature_cache)) if args.raw_feature_cache else "",
            "qa_feature_cache": str(resolve_cache_path(args.qa_feature_cache)) if args.qa_feature_cache else "",
            "vendored_qa_feature_metadata": qa_feature_metadata,
        },
        "datasets": {
...
def main():
    ...
    if not args.raw_feature_cache:
        args.raw_feature_cache = str(Path("outputs") / f"{dataset_name}_vendored_ablation" / "raw_feature.pkl")
    if not args.qa_feature_cache:
        args.qa_feature_cache = str(Path("outputs") / f"{dataset_name}_vendored_ablation" / "qa_feature.pkl")
    if not args.output:
        args.output = str(Path("outputs") / f"{dataset_name}_vendored_ablation" / "summary.json")
    ...
    print(f"cwd={Path(os.getcwd()).resolve()}", flush=True)
    ...
    raw_feature_df, qa_feature_df, qa_feature_metadata = build_feature_matrices(factors, field_panels, args)
    ...
    summary = build_summary(
        ...
        qa_feature_metadata=qa_feature_metadata,
    )
